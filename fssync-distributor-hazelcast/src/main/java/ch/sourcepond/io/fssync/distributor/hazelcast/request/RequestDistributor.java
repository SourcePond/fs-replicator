...
public class RequestDistributor {
    ...
    public void transfer(final String pSyncDir, final String pPath, final ByteBuffer pData) throws TransferException {
        ...
        try {
            ...
            clusterResponseBarrierFactory.create(pPath, transferRequestTopic).awaitResponse(
                    new TransferRequest(pSyncDir, pPath, data));
    ...
    public void discard(final String pSyncDir, final String pPath, final IOException pFailureOrNull) throws DiscardException {
        try {
            clusterResponseBarrierFactory.create(pPath, discardRequestTopic).awaitResponse(
                    new StatusMessage(pSyncDir, pPath, pFailureOrNull));
        } catch (final TimeoutException | ResponseException e) {
            throw new DiscardException(format("Storing or reverting %s failed on some node!", pPath), e);
        }
    }

    public void store(final String pSyncDir, final String pPath) throws StoreException {
        try {
            clusterResponseBarrierFactory.create(pPath, storeRequestTopic).awaitResponse(
                    new StatusMessage(pSyncDir, pPath));
        } catch (final TimeoutException | ResponseException e) {
