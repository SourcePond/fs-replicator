...
public class StoreRequestProcessorTest extends ClientMessageProcessorTest<StatusMessage, StoreRequestProcessor> {
    ...
    public void processMessage() throws IOException {
        ...
        verify(receiver).store(path, null);
    }

    @Test
    public void processMessageWithFailure() throws IOException {
        final IOException expected = new IOException();
        when(message.getFailureOrNull()).thenReturn(expected);
        processor.processMessage(path, message);
        verify(receiver).store(path, expected);
    }
}
